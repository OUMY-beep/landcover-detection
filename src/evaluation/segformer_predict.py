"""
Run a trained SegFormer model on a single image or on a full dataset split,
...
DEFAULT_CHECKPOINT = CHECKPOINT_DIR / "segformer_best.pth"


def load_model(checkpoint_path: Path = DEFAULT_CHECKPOINT, device: torch.device = DEVICE) -> SegFormer:
    """
    Build a SegFormer and load trained weights from a checkpoint.
    """
...
@torch.no_grad()
def predict_image(model: SegFormer, image_path: Path, device: torch.device = DEVICE) -> torch.Tensor:
    """
    ...
    logits = model(image)  # already upsampled to input resolution by SegFormer.forward
...
@torch.no_grad()
def predict_dataset(model: SegFormer, loader, device: torch.device = DEVICE):
    """
